...
(function () {
  ...
  UI.showTower = function (t) {
    ...
    if (s.dmg) row('Damage', TD.fmt(Math.round(s.dmg * (1 + t.buff.dmg))));
    if (s.cd) row('Rate', (1 / (s.cd / (1 + t.buff.rate))).toFixed(2) + '/s');
    if (s.range) row('Range', Math.round(s.range * (1 + t.buff.range)));
    if (s.dmg && s.cd) row('DPS', TD.fmt(TD.towerDps(t.def, t.level)));
